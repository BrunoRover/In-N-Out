...
class WorkingHours extends Model{
    ...
    public function innout($time){
        ...
        $this->worked_time = getSecondsFromDateInterval($this->getWorkdInterval());
        if($this->id){
            $this->update();
        }else{
            $this->insert();           
    ...
    public static function getAbsentUsers(){
        ...
        if($result->num_rows > 0){
            while($row = $result->fetch_assoc()){
                array_push($absentUser, $row['name']);
            }
        }
    ...
    public static function getWorkedTimeInMonth($yearAndMonth){
        ...
        $result = static::getResultFormSelect([
